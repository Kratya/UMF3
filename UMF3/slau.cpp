...
void SLAU::read_cond(string cond_file_1, string cond_file_2) {
...
void SLAU::mult_pr(vector<double> aa, vector<double>& y, vector<double> bb) {
...
void SLAU::mult_obr(vector<double> aa, vector<double>& y, vector<double> bb) {
	for (int i = 0; i < y.size(); i++)
		y[i] = bb[i];
...
void SLAU::multU(vector<double>& MV, vector<double>vec) {
	for (int i = 0; i < vec.size(); i++) {
		int k0 = ig[i];
		int k1 = ig[i + 1];
		MV[i] = di[i] * vec[i];
